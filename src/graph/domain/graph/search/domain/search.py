# src/logic/graph/searcher/points/searcher

"""
Module: logic.graph.searcher.points.searcher
Author: Banji Lawal
Created: 2025-11-09
version: 1.0.0
"""
...
from graph.domain.graph import Graph, GraphDomainFilter, GraphValidator
...
class GraphDomainFinder(Finder[Graph, Domain]):
    ...
    def search(cls, data_owner: Graph, search_context: GraphDomainFilter) -> SearchResult[List[Domain]]:
        ...
        try:
            graph_validation = GraphValidator.search_service(data_owner)
            if graph_validation.is_failure():
                return SearchResult.failure(graph_validation.exception)
            
            filter_validation = GraphSearchContextValidator.search_service(search_context)
    ...
    def _name_search(cls, graph: Graph, name: str) -> SearchResult[List[Domain]]:
        ...
        try:
            matches = [domain for domain in graph.domains if domain.model_class.name.upper == name.upper()]
            if len(matches) == 0:
                return SearchResult.empty()
            elif len(matches) == 1:
                return SearchResult.success(matches)
    ...
    def _rank_name_search(cls, graph: Graph, name: str) -> SearchResult[List[Domain]]:
        ...
        try:
            matches = [domain for domain in graph.domains if domain.ownerr.rank_level.name.upper() == name.upper()]
            if len(matches) == 0:
                return SearchResult.empty()
    ...
    def _team_name_search(cls, graph: Graph, name: str) -> SearchResult[List[Domain]]:
        ...
        try:
            matches = [domain for domain in graph.domains if domain.model_class.team.name.uppper() == name.upper()]
            if len(matches) == 0:
                return SearchResult.empty()
