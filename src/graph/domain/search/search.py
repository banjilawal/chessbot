...
class DomainResidentFinder(Finder[Domain, Piece]):
    """"""
    ...
    def search(cls, data_owner: Domain, search_context: ResidentFilter) -> SearchResult[List[Piece]]:
        ...
        try:
            ...
            search_context_validation = ResidentFilterValidator.execute(search_context)
    ...
    def _ransom_search(cls, domain: Domain, ransom: int) -> SearchResult[List[Piece]]:
        ...
        try:
            matches = [resident for resident in domain.residents if resident.rank_level.ransom == ransom]
            if len(matches) == 0:
                return SearchResult.empty()
