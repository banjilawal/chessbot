# src/recurrence/registry/axis/registry.py

"""
Module: recurrence.registry.axis.space
Author: Banji Lawal
Created: 2026-04-03
version: 1.0.1
"""
...
class AxisRecurrenceRegistry(RecurrenceRegistry[Axis]):
    ...
    _recurrence_registry: Dict[str, AxisRecurrence]
    ...
    def __init__(
        ...
        self._recurrence_registry["east"] = EastAxisRecurrence(
                space=space,
    ...
    def east_axis_recurrence(self) -> EastAxisRecurrence:
        return cast(
            EastAxisRecurrence,
            self._recurrence_registry["east"]
        )
    ...
    def north_axis_recurrence(self) -> NorthAxisRecurrence:
        return cast(
            NorthAxisRecurrence,
            self._recurrence_registry["north"]
        )
    ...
    def south_axis_recurrence(self) -> SouthAxisRecurrence:
        return cast(
            SouthAxisRecurrence,
            self._recurrence_registry["south"]
        )
    ...
    def west_axis_recurrence(self) -> WestAxisRecurrence:
        return cast(
            WestAxisRecurrence,
            self._recurrence_registry["west"]
        )
