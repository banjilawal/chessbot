# src/pattern/adder/signature.py

"""
Module: pattern.adder.signature
Author: Banji Lawal
Created: 2026-04-03
version: 1.0.1
"""
...
class OffsetTopologyGenerator:
    ...
    def execute(
        ...
        # Handle the case that, the signature is not valid.
        signature_validation = self._priming_validator.execute(
            candidate=signature_vectors,
            target=Type[VectorSet],
            null_exception=VectorSetNullException(),
        )
        if signature_validation.is_failure:
            # Send an exception chain in the result.
            return ComputationResult.failure(
                TopologyGeneratorException(
                    cls_mthd=method,
                    ...
                    err_code=TopologyGeneratorException.ERR_CODE,
                    mthd_rslt_type=MethodResultType.COMPUTATION_RESULT,
                    ex=signature_validation.exception,
                ),
            )
        root = cast(Vector, origin_validation.payload)
        offsets = cast(VectorSet, signature_validation.payload)
