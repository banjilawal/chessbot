# src/certifier/node/validator.py

"""
Module: certifier.node.validator
Author: Banji Lawal
Created: 2026-04-03
version: 1.0.1
"""
...
class NodeRootCertifier(RootCertifier[Node]):
    """
    ...
    def validate(
        ...
        if candidate is None:
            ...
            return ValidationResult.failure(
                NodeCertifierException(
                    msg=f"{method}: {NodeCertifierException.MSG}",
        ...
        if blueprint.home_square is not None:
            validation = square_service.run.execute(blueprint.home_square)
            if validation.is_failure:
                # Send the exception chain on failure.
                return ValidationResult.failure(
                    NodeCertifierException(
