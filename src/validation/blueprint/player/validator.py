# src/validation/blueprint/player/validator.py

"""
Module: validation.blueprint.player.validator
Author: Banji Lawal
Created: 2026-04-03
version: 1.0.1
"""
...
class PlayerBlueprintValidator(BlueprintValidator[Player]):
    """
    ...
    def validate(
        ...
        try:
            ...
            if blueprint.team is not None:
                validation = team_service.validate.build(candidate=blueprint.team)
                if validation.is_failure:
                    return ValidationResult.failure(validation.exception)
                return ValidationResult.success(blueprint)
            
            if blueprint.game is not None:
                validation = game_service.validate.build(candidate=blueprint.game)
                if validation.is_failure:
                    return ValidationResult.failure(validation.exception)
                return ValidationResult.success(blueprint)
