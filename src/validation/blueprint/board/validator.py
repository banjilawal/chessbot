# src/validation/blueprint/board/validator.py

"""
Module: validation.blueprint.board.validator
Author: Banji Lawal
Created: 2026-04-03
version: 1.0.1
"""
...
class BoardBlueprintValidator(BlueprintValidator[Board]):
    ...
    def validate(
        ...
        if blueprint.arena is not None:
            validation = arena_service.validate.search_service(blueprint.arena)
            if validation.is_failure:
                # Send the exception chain on failure.
                return ValidationResult.failure(
                    BoardBlueprintValidationException(
