# src/validation/blueprint/game/validator.py

"""
Module: validation.blueprint.game.validator
Author: Banji Lawal
Created: 2026-04-03
version: 1.0.1
"""
...
class GameBlueprintValidator(BlueprintValidator[Game]):
    """
    ...
    def validate(
        ...
        try:
            ...
            if blueprint.agent is not None:
                validation = agent_service.validate.search_service(candidate=blueprint.agent)
                if validation.is_failure:
                    return ValidationResult.failure(validation.exception)
