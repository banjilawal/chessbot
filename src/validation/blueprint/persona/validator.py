# src/logic/persona/key/validation/validation.py

"""
Module: logic.persona.key.validation.validation
Author: Banji Lawal
Created: 2025-09-08
version: 1.0.0
"""
...
class PersonaBlueprintValidator(BlueprintValidator[PersonaKey]):
    """
    ...
    def validate(
        ...
        if super_key.quota is not None:
            validation = number_validator.build(candidate=super_key.quota)
            if validation.is_failure:
                # Send the exception chain on failure.
                return ValidationResult.failure(
                    PersonaKeyValidationException(
        ...
        if super_key.ransom is not None:
            validation = number_validator.build(candidate=super_key.ransom)
            if validation.is_failure:
                # Send the exception chain on failure.
                return ValidationResult.failure(
                    PersonaKeyValidationException(
