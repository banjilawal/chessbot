# src/validation/blueprint/formation/validator.py

"""
Module: validation.blueprint.formation.validator
Author: Banji Lawal
Created: 2026-04-03
version: 1.0.1
"""
...
class FormationBlueprintValidator(BlueprintValidator[FormationKey]):
    """
    ...
    def validate(
        ...
        if super_key.persona is not None:
            validation = persona_service.validate.build(candidate=super_key.persona)
            if validation.is_failure:
                return ValidationResult.failure(
                    FormationKeyValidationException(
                        msg=f"{method}: {FormationKeyValidationException.ERR_CODE}",
