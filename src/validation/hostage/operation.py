...
class HostageValidator(Validator[Hostage]):
    ...
    def validate(
        ...
        captured_square_validation = square_service.validate.build(candidate=manifest.captured_square)
        ...
        victor_square_validation = square_service.validate.build(candidate=manifest.victor_square)
        ...
        prisoner_validation = token_service.validate.verify_token_is_combatant(candidate=manifest.prisoner)
        ...
        victor_validation = token_service.validate.search_service(candidate=manifest.victor)
        ...
        if manifest.victor.deployment_state == TokenBoardState.NOT_DEPLOYED:
            # Send the exception chain on failure
            return ValidationResult.failure(
                HostageValidationException(
                    msg=f"{method}: {HostageValidationException.MSG}",
