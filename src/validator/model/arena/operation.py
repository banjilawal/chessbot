...
from logic.arena import Arena, ArenaValidatorException, NullArenaException, ArenaTeamsInArenaException
from system import (
...
class ArenaValidator(ModelValidator[Arena]):
    ...
    def validate(
        ...
        try:
            ...
            if id_validator.failure:
                return ValidationResult.failure(id_validator.exception)
             
            # Verify the board.
            board_validation = board_service.run.build(arena.board)
            if board_validator.failure:
            ...
            if team_service_validator.failure:
                return ValidationResult.failure(team_service_validator.exception)
        ...
        # In that case wrap the unhandled exception inside an ArenaValidatorException then, return
        # the exception chain inside a ValidationResult.
        except Exception as ex:
            return ValidationResult.failure(
                ArenaValidatorException(
                    ex=ex, msg=f"{method}: {ArenaValidatorException.MSG}"
                )
            )