...
class EdgeContextValidator(ContextValidator[Edge]):
    ...
    def validate(
        ...
        if context.coord is not None:
            validation = coord_service.execute.build(context.coord)
            if validator.is_failure:
                # Send the exception chain on failure.
                return ValidationResult.failure(
                    EdgeContextValidationException(
        ...
        if context.board is not None:
            validation = board_service.execute.build(context.board)
            if validator.is_failure:
                # Send the exception chain on failure.
                return ValidationResult.failure(
                    EdgeContextValidationException(
        ...
        if context.occupant is not None:
            validation = edge_service.execute.build(context.occupant)
            if validator.is_failure:
                # Send the exception chain on failure.
                return ValidationResult.failure(
                    EdgeContextValidationException(
