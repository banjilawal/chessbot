# src/logic/board/context/validation/validation.py

"""
Module: logic.board.context.validation.validation
Author: Banji Lawal
Created: 2025-11-22
version: 1.0.0
"""
...
class BoardContextValidator(Validator[BoardContext]):
    ...
    def validate(
        ...
        if context.arena is not None:
            validation = arena_service.validator.search(context.arena)
            if validation.is_failure:
                # Return the exception chain on failure.
                return ValidationResult.failure(
                    BoardContextValidationException(
