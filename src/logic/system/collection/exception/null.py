# src/logic/system/collection/exception/null.py

"""
Module: logic.system.collection.exception.null
Author: Banji Lawal
Created: 2025-11-18
Version: 1.0.0
"""


__all__ = [
    # ====================== NULL_COLLECTION EXCEPTION #======================#
    "NullCollectionException",
]

from logic.system import CollectionException, NullException


# ====================== NULL_COLLECTION EXCEPTION #======================#
class NullCollectionException(CollectionException, NullException):
    """
    Role:Error Tracing, Debugging

    Responsibilities:
    1.  A debug exception is created when a Team rank fails a validation test. Validation debug exceptions are
        encapsulated inside an TeamValidationException creating an exception chain. which is sent to the caller in a
        ValidationResult.
    3.  Super for errors not covered by lower level NullCollectionException subclasses.

    Super Class:
     *   NullException
     *   CollectionException

    Provides:


    # INHERITED ATTRIBUTES:
    None
    """
    ERR_CODE = "NULL_COLLECTION_EXCEPTION"
    MSG = "Collection cannot be null."