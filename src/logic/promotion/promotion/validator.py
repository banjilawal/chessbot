# src/logic/chess/owner/travel/occupation/exception.py
...
from logic.board import BoardContext, BoardSquareFinder
from logic.board.search.context.builder import BoardContextBuilder
from logic.pawn.promotion.event import PromotionEvent
...
class OldPromotionEventValidator(Validator[PromotionEvent]):
    ...
    def validate(cls, candidate: Any) -> ValidationResult[OccupationEvent]:
        ...
        try:
            ...
            actor_validator = BoardActorValidator.query(event.actor, event.execution_environment)
            ...
            context_build_result = BoardContextBuilder.query(piece_id=event.actor.visitor_id)
            ...
            resource_validation = TravelResourceValidator.query(event.square, event.execution_environment)
