...
from domain.model.state.token import Discovery
...
class ScoutReportAnalyzer:
  ...
  def _sort_obstructions(self) -> List[Discovery]:
    ...
    for square in self._scout_report.squares:
      ...
      if occupant is not None and not self._scout.is_enemy(occupant):
        obstruction = Discovery(occupant)
  ...
  def _sort_enemies(self) -> List[Piece]:
    ...
    enemies.sort(
      reverse=True,
      key=lambda chess_piece: chess_piece.rank_name.visitor_ransom
    )
    return enemies
