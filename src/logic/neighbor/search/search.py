...
class VisitationFinder(Finder[List[Coord]]):
  """"""
  ...
  def search(cls, owner: Visitation, search_context: VisitationSearchContext) -> SearchResult[List[Coord]]:
      ...
      visitation_validation = VisitationValidator.execute(owner)
      if not visitation_validation.is_success():
        return SearchResult.failure(visitation_validation.exception)

      search_context_validation = VisitationSearchContextValidator.execute(search_context)
      ...
      if search_context.name is not None:
        return VisitationFinder._name_search(visitation=owner, name=search_context.name)

      if search_context.rank is not None:
        return VisitationFinder._rank_filter(visitation=owner, rank=search_context.rank)

      if search_context.ransom is not None:
        return VisitationFinder._ransom_filter(visitation=owner, ransom=search_context.ransom)

      if search_context.piece_id is not None:
        return VisitationFinder._id_search(visitation=owner, piece_id=search_context.piece_id)

      if search_context.roster_number is not None:
        return VisitationFinder. _roster_number_search(visitation=owner, roster_number=search_context.roster_number)
  ...
  def _name_search(cls, visitation: Visitation, name: str) -> SearchResult[List[Coord]]:
      ...
      """
      method = "VisitationFinder._name_search"
  ...
  def _id_search(cls, visitation: Visitation, piece_id: int) -> SearchResult[List[Coord]]:
      ...
      """
      method = "VisitationFinder._id_search"
  ...
  def _roster_number_search(cls, visitation: Visitation, roster_number: int) -> SearchResult[List[Coord]]:
      ...
      """
      method = "VisitationFinder._roster_number_search"
