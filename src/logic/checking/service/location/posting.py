...
class KingLocationPostingService(Service[KingOccupationEvent]):
    ...
    def process_request(self, request: KingOccupationEvent) -> TranactionResult[KingLocationRecord]:
        ...
        if validation_result.is_exception():
        ...
        check_posting_search_result = CheckPostingService.route(king=request.actor, location=request.destination_square)
        ...
        build_result = KingLocationRecordBuilder.execute(event=request)
