...
class KingOccupationEventValidationTransaction(ValidationTransaction[KingOccupationEvent]):
    """"""
    
    @classmethod
    def execute(cls, candidate: Any) -> ValidationResult[KingOccupationEvent]:
        """"""
        method = "KingOccupationEventValidationTransaction.validate"
        
        try:
            if candidate is None:
                return ValidationResult(exception=NullKingOccupationEventException(
                    f"{method}: {NullKingOccupationEventException.MSG}"
                    )
                )
...
#       rollback_exception=KingOccupationEventException(f"{method}: {KingOccupationEventException.MSG}"),
#     )
#
