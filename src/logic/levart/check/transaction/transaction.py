...
from model.state.token import SetCaptorRolledBackException, \
  EmptyDestinationSquareRolledBackException
from model.state.token import OccupationTransaction
...
class CheckTransaction(TravelTransaction[KingCheckEvent]):
  ...
  def execute(self) -> TransactionResult:
    ...
    validation = AttackEventValidator.search_service(self.event)
    ...
    return OccupationTransaction.search_service(transfer_event, context)
