...
class BlockingEventValidator(Validator[BlockingEvent]):
    ...
    def validate(cls, candidate: BlockingEvent) -> ValidationResult[BlockingEvent]:
        ...
        try:
            ...
            actor_validation = BoardActorValidator.search_service(
                actor=event.actor,
            ...
            blocker_validation = PieceValidator.search_service(event.friend)
            ...
            context_build_result = DiscoverySearchContextBuilder.search_service(piece_id=event.friend.visitor_id)
