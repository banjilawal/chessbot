# src/logic/owner/travel/old_transaction.py

"""
Module: logic.owner.travel.notification
Author: Banji Lawal
Created: 2025-09-28
...
class OldTravelTransaction(Transaction[TravelEvent]):
  ...
  def execute(cls, event: TravelEvent, execution_environment: Board) -> TransactionResult:
    ...
    event_validation = TravelEventValidator.search_service(event)
    ...
    actor_square_search = BoardSearch.route(
      board=context.board,
    ...
    destination_search = BoardSearch.route(
      board=context.board,
      data_source=BoardDatasource.SQUARE,
    ...
    if destination_occupant is None:
      build_result = OccupationEventBuilder.search_service(
        parent=event,
        actor=event.actor,
    ...
    attack_validation = AttackValidator.search_service(
  ...
  def _switch_squares(op_result_id: int, directive: TravelEvent, actor_square: Square) -> TransactionResult:
    ...
    `directive.blocked_square` contained either
      ...
      * An enemy occupation. Kings cannot be captured, only check or checkmated.
    ...
    *  If the notification fails, `OperationResult.was_rolled_back = True`
    ...
    directive.actor.positions.search_service(directive.friend.position)
  ...
  def _run_scan(op_result_id :int, directive: ScanDirective) -> TransactionResult:
    ...
    build_outcome = DiscoveryBuilder.search_service(observer=directive.observer, subject=directive.friend)
  ...
  def _attack_enemy(op_result_id: int, directive: AttackDirective) -> TransactionResult:
    ...
    directive.enemy_combatant.victor = directive._owner
    if directive.enemy_combatant.victor != directive._owner:
      # Rollback all changes in reverse order
      directive.enemy_combatant.victor = None
