# src/transit/carrier/model/path/carrier.py

"""
Module: transit.carrier.model.path.carrier
Author: Banji Lawal
Created: 2026-04-03
version: 0.0.2
"""

from __future__ import annotations

from typing import Optional, cast

from domain import Path, PathBlueprint
from transit import ModelCarrier


class PathCarrier(ModelCarrier[Path]):
    """
    Role:
        - Boundary Carrier Interface

    Responsibilities:
        1.  Transport a hydrated Path or its Blueprint across processing boundaries.

    Attributes:
        size: int
        is_empty: bool
        is_over_capacity: bool
        is_model_carrier: bool
        is_blueprint_carrier: bool
        entity: [Path|PathBlueprint]

    Provides:
        - def extract_blueprint() -> Optional[PathBlueprint]

    Super Class:
        ModelCarrier
    """
    
    _model: Optional[Path]
    _blueprint: Optional[PathBlueprint]
    
    def __init__(
            self,
            model: Optional[Path] | None = None,
            blueprint: Optional[PathBlueprint] | None = None,
    ):
        """
        Args:
            model: Optional[Path]
            blueprint: Optional[PathBlueprint]
        """
        super().__init__()
        self._model = model
        self._blueprint = blueprint
    
    @property
    def entity(self) -> Optional[Path|PathBlueprint]:
        if self.is_empty:
            return None
        if self.is_carrying_model:
            return self._model
        return self._blueprint
    
    @property
    def is_carrying_model(self) -> bool:
        return (
                self._model is not None and
                self._blueprint is None and
                isinstance(self._model, Path)
        )
    
    @property
    def is_carrying_blueprint(self) -> bool:
        return (
                not self.is_carrying_model and
                isinstance(self._blueprint, PathBlueprint)
        )
    
    @property
    def size(self) -> int:
        return len([self._model, self._blueprint])
    
    @property
    def is_empty(self) -> bool:
        return self.size == 0
    
    @property
    def is_over_capacity(self) -> bool:
        return self.size > 1
    
    def extract_blueprint(self) -> Optional[PathBlueprint]:
        if self.is_empty: return None
        if self.is_carrying_blueprint: return self._blueprint
        
        model = cast(Path, self._model)
        return PathBlueprint(
            label=model.label,
            endpoints=model.endpoints,
        )

