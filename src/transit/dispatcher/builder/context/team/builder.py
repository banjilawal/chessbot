# src/transit/dispatcher/builder/context/team/dispatcher/builder.py

"""
Module: transit.dispatcher.builder.context.team.builder
Author: Banji Lawal
Created: 2026-04-03
version: 0.0.2
"""
...
class TeamContextBuilder(Builder[TeamContext]):
   ...
   Responsibilities:
    ...
    def build(
        ...
        """
        method = "PieceContextBuilder.build"
        ...
        if player is not None:
            validation = player_service.execute.execute(candidate=player)
            if validation.is_failure:
                # Send the exception chain on failure.
                return BuildResult.failure(
                    TeamContextBuilderException(
