# src/transit/dispatcher/builder/context/token/dispatcher/builder.py

"""
Module: transit.dispatcher.builder.context.token.builder
Author: Banji Lawal
Created: 2026-04-03
version: 0.0.2
"""
   ...
   Responsibilities:
    ...
    def build(
        ...
        if current_position is not None:
            validation = workers.coord_service.execute.execute(current_position)
            if validation.is_failure:
                # Send the exception chain on failure.
                return BuildResult.failure(
                    TokenContextBuilderException(
        ...
        if rank is not None:
            validation = workers.rank_service.execute.execute(rank)
            if validation.is_failure:
                # Send the exception chain on failure.
                return BuildResult.failure(
                    TokenContextBuilderException(
        ...
        if team is not None:
            validation = workers.team_service.execute.execute(team)
            if validation.is_failure:
                # Send the exception chain on failure.
                return BuildResult.failure(
                    TokenContextBuilderException(
