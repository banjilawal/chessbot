...
class TeamBuilder(Builder[Team]):
   ...
   Responsibilities:
    ...
    def _validate_build_params(
        ...
        schema_validation = schema_service.execute.build(candidate=schema)
        ...
        owner_validation = player_service.execute.build(candidate=owner)
        ...
        board_validation = board_service.execute.build(candidate=board)
        if owner_validation.is_failure:
            # Send the exception chain on failure.
