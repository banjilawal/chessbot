# src/operation/toolkit/context/player/toolkit.py

"""
Module: operation.toolkit.context.player.toolkit
Author: Banji Lawal
Created: 2026-04-03
version: 0.0.2
"""
...
from domain.model.state.game import Game, GameService
   ...
   Responsibilities:
    ...
    def __init__(
        ...
        Raises:
            ...
            *   ArenaPlayerContextToolkitFlagsException
        """
        method = "AgentSearchContextTooltoolkit"
        try:
            ...
            if team is not None:
                validation = team_service.execute.execute(candidate=team)
                if validation.is_failure:
                    return ToolkitResult.failure(validation.exception)
            ...
            if game is not None:
                validation = game_service.execute.execute(candidate=game)
                if validation.is_failure:
                    return ToolkitResult.failure(validation.exception)
