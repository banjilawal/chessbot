...
class TokenMicroservice(MicroserviceOperation(Operation, ABC, Generic[T]):
    ...
    def __init__(self, id: int, permitter: TokenMicroservicePermitter[T]):
        """
        Args:
            permitter: MicroserviceOperationPermitter[T]
        """
    ...
    @property
    def permitter(self) -> TokenMicroservicePermitter[T]:
        return cast(TokenMicroservicePermitter[T], super().permitter)
    
    
    @abstractmethod
    @LoggingLevelRouter.monitor
    def execute(self, request: TokenMicroserviceRequest[T]) -> T:
        pass