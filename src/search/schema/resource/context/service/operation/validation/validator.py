...
class SchemaContextValidator(Validator[SchemaContext]):
    ...
    def validate(
        ...
        if context.current_position is not None:
            validation_result = workers.coord_service.validator.build(
        ...
        if context.team is not None:
            validation_result = workers.team_service.validator.build(
                rank=context.team
            )
            if validation_result.is_failure:
                # Send the exception chain on failure.
        ...
        if context.rank is not None:
            validation_result = workers.rank_service.validator.build(
                rank=context.rank
            )
            if validation_result.is_failure:
                # Send the exception chain on failure.
