...
from domain.exchange.model import GameService
    ...
    def validate(
        ...
        try:
            ...
            if context.game is not None:
                validation = game_service.execute.execute(candidate=context.game)
            ...
            if context.team is not None:
                validation = team_service.execute.execute(candidate=context.team)
                if validation.is_failure:
                    return ValidationResult.failure(validation.exception)
            ...
            if context.arena is not None:
                validation = arena_service.execute.search_service(candidate=context.arena)
                if validation.is_failure:
                    return ValidationResult.failure(validation.exception)
            ...
            if context.plyer is not None:
                validation = player_service.execute.search_service(candidate=context.domain_class)
                if validation.is_failure:
                    return ValidationResult.failure(validation.exception)
