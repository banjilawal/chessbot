# src/domain/exchange/request/operation/square/entry/operation.py

"""
Module: domain.exchange.request.operation.square.entry.operation
Author: Banji Lawal
Created: 2026-04-03
version: 0.0.2
"""
...
from domain.exchange.model.state.token import Token, DeploymentState, TokenFreedomAnalyzer
...
class SquareEntry:
    ...
    def _register_token_visit(cls, square: Square, token: Token) -> UpdateResult[Square]:
        ...
        if token.deployment == DeploymentState.NOT_DEPLOYED:
            token.deployment = DeploymentState.DEPLOYED_TO_HOME_SQUARE
