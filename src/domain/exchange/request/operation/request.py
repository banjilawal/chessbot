# src/domain/exchange/request/operation/request.py

"""
Module: domain.exchange.request.operation.request
Author: Banji Lawal
Created: 2026-04-03
version: 0.0.2
"""

from __future__ import annotations

from abc import ABC
from typing import Generic, TypeVar, cast

from authorization import Request

T = TypeVar("T", bound="Operation")


class OldRequest(Request, ABC, Generic[T]):
    """
     Role:
         -  Messaging

     Responsibilities:
         1. Transport job information throughout the Operation lifecycle.

     Attributes:

     Provides:
     
     Super Class:
        Request
     """
    
    def __init__(self, id: int):
        """
        Args:
            id: int
        """
        super().__init__(id=id)
    
    def __eq__(self, other):
        if other is self: return True
        if other is None: return False
        if isinstance(other, OperationRequest):
            request = cast(OperationRequest, other)
            return self.id == domain.exchange.request.id
        return False