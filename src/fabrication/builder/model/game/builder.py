# src/fabrication/builder/model/game/fabrication/builder.py

"""
Module: fabrication.builder.model.game.builder
Author: Banji Lawal
Created: 2026-04-03
version: 1.0.1
"""

from __future__ import annotations

from typing import Optional, cast

from fabrication.blueprint import GameBlueprint
from fabrication.builder import ModelBuilder
from err import GameBuilderException
from model import Game
from result import BuildResult, MethodResultType
from kit.toolkit import GameBuilderToolkit
from util import LoggingLevelRouter


class GameBuilder(ModelBuilder[Game]):
    """
    Role
        -   Build Pipeline
        -   Integrity Management
        -   Consistency Assurance
        -   Workflow Owner

   Responsibilities:
        1.  Ensure a new Game instance is born safe and reliable.

    Attributes:
            builder_toolkit: Optional[GameBuilderToolkit]

    Provides:
        -   def execute(self, blueprint: GameBlueprint) -> BuildResult[Game]

     Super Class:
         ModelBuilder
     """
    
    def __init__(
            self,
            builder_toolkit: Optional[GameBuilderToolkit] | None = GameBuilderToolkit(),
    ):
        """
        Args:
            builder_toolkit: Optional[GameBuilderToolkit]
        """
        super().__init__(builder_toolkit=builder_toolkit)
    
    @property
    def builder_toolkit(self) -> GameBuilderToolkit:
        return cast(GameBuilderToolkit, super().builder_toolkit)
    
    
    @LoggingLevelRouter.monitor
    def execute(self, blueprint: GameBlueprint) -> BuildResult[Game]:
        """
        Build a safe Game.

        Action:
            1.  Send an exception chain in the BuildResult if either
                    -   The GameBlueprint object is flagged unsafe.
                    -   The assembler does not return a product.
            2.  Otherwise, cast the assembler product as a Game then, send in the success result,
        Args:
            blueprint: GameBlueprint
        Returns:
            BuildResult[Game]
        Raises:
            GameBuilderException
        """
        method = f"{self.__class__.__name__}.build"
        
        # Handle the case that, the blueprint is not certified safe.
        blueprint_validation = self.builder_toolkit.root_certifier.execute(
            candidate=blueprint
        )
        if blueprint_validation.is_failure:
            # Send the exception chain on failure.
            return BuildResult.failure(
                GameBuilderException(
                    cls_mthd=method,
                    cls_name=self.__class__.__name__,
                    msg=GameBuilderException.MSG,
                    err_code=GameBuilderException.ERR_CODE,
                    mthd_rslt_type=MethodResultType.BUILD_RESULT,
                    ex=blueprint_validation.exception
                )
            )
        # --- Handoff the validated blueprint to the assembler. ---#
        assembly = self.builder_toolkit.assembler.execute(
            blueprint=cast(
                GameBlueprint,
                blueprint_validation.payload
            )
        )
        # Handle the case that assembler cannot satisfy the product request.
        if blueprint_validation.is_failure:
        # Send the exception chain on failure.
            return BuildResult.failure(
                GameBuilderException(
                    cls_mthd=method,
                    cls_name=self.__class__.__name__,
                    msg=GameBuilderException.MSG,
                    err_code=GameBuilderException.ERR_CODE,
                    mthd_rslt_type=MethodResultType.BUILD_RESULT,
                    ex=blueprint_validation.exception
                )
            )
        # --- Forward the work product to the caller. ---#
        return BuildResult.success(cast(Game, assembly.payload))