...
class DiscoveryBuilder(Enum):
  ...
  `DiscoveryBuilder` ensures that `Discovery` objects are always created successfully by performing comprehensive validate
  ...
  The build runs through all validate checks individually to guarantee that any `Discovery` instance it produces
  ...
  def build(observer: Piece, subject: Piece) -> BuildResult[Discovery]:
    ...
    Args:
      `actor`(`Piece`): The piece that is doing team encounter, or executing team move.
    ...
    Raises:
      ...
      process. This includes:
        * `PieceValidationException`: if `actor` or `discover` fails validate checks
