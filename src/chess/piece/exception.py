...
class PieceMissingDiscoveriesException(PieceException, InconsistencyException):
    """
    Raised if piece.discovery list does not exist. If the piece.discoveries == validation there is service inconsistency
    or loss.
    """
    ERROR_CODE = "PIECE_DISCOVERY_LIST_MISSING_ERROR"
    DEFAULT_MESSAGE = "Piece.discovery list is validation. It should never be validation. There may be service inconsistency or loss."
