...
class DiscoveryBuilder(Enum):
    ...
    @staticmethod
    def build(observer: Piece, discovery: Piece) -> BuildResult[Discovery]:
        """
        ...
        try:
            ...
            if observer == discovery:
                ThrowHelper.throw_if_invalid(
                    DiscoveryBuilder,
                    AutoDiscoveryException(AutoDiscoveryException.DEFAULT_MESSAGE)
                )
                
            return BuildResult(payload=Discovery(piece=cast(Piece, discovery)))
