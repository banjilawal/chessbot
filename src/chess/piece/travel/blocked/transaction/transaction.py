...
from chess.piece import Discovery, DiscoveryBuilder, BlockingEvent
...
class FriendEncounterTransaction(OccupationTransaction[BlockingEvent]):
  ...
  def execute(event: BlockingEvent) -> TransactionResult:
    ...
    try:
      ...
      build_result = DiscoveryBuilder.build(observer=event.observer, subject=event.friend)
