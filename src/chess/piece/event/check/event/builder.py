  ...
  def build(
    ...
    try:
      ...
      if not actor_validation.is_success():
        raise InvalidAttackException(f"{method}: CheckEvent actor_candidate failed validate")

      enemy_validation = PieceValidator.validate(enemy)
      if not enemy_validation.is_success():
        raise InvalidAttackException(f"{method}: CheckEvent enemy failed validate")
