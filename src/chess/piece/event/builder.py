...
from chess.piece import Piece, PieceValidator, OccupationEvent, EncounterEvent, AttackEvent, CombatantPiece, KingPiece
from chess.piece.event.exception import TravelEventResourceNotFoundException, EventActorSquareNotFoundException, \
  TravelEventActorNotFoundException, ActorNotOnBoardCannotMoveException, ActorNotOnRosterCannotMoveException
from chess.rank import King
from chess.system import Transaction, TransactionResult, TransactionState, LoggingLevelRouter, \
  SearchResult, Builder, BuildResult, InvalidIdException, CapturedActorCannotMoveException
...
class TravelEventBuilder(Builder[TravelEvent]):
  ...
  def build(cls, actor: Piece, destination_square: Square, board: Board) -> BuildResult[TravelEvent]:
    ...
    if actor_square == destination_square:
      return BuildResult(exception=AAutoTravelPieceException(
