...
class HumanPlayerBuilder:
    ...
    def build_player(name: str, player_config: PlayerConfig) -> HumanPlayer:
        ...
        This builder is fully responsible for creating all pieces for the player,
        avoiding delegation of piece creation to the Player object itself.
        """
        # 1. Create the HumanPlayer object
        player = HumanPlayer(
            player_id=id_emitter.player_id, # Get a unique ID for this player
            name=name,
            color=player_config.game_color
        )

        # 2. Directly create and assign pieces to this player.
        ...
        for _ in range(2):
            ...
            piece.player = player # The piece's setter adds it to player.pieces and rank.members
