# src/chess/rank/rook/validator_.py

"""
Module: chess.rank.rook.validator
Author: Banji Lawal
Created: 2025-09-08
version: 1.0.0
"""
...
class RookValidator(Validator[Rook]):
    ...
    def verify_id(
        ...
        2.  Check if id != RankSpec.ROOK.id. If not, return failure.
    ...
    def verify_name(
        ...
        2.  Check if name != RankSpec.ROOK.name. If not, return failure.
        3.  If all checks pass the id in a success validation result.
        ...
        try:
            ...
            if validation.is_failure():
                return ValidationResult.failure(validation.exception)
            # Next check if name is correct for a rook.
            ...
            # If no errors are detected send the verified rook.name inside a ValidationResult.
    ...
    def verify_ransom(
        ...
        2.  Check if ransom != RankSpec.ROOK.ransom. If not, return failure.
        3.  If all checks pass the id in a success validation result.
    ...
    def verify_designation(
        ...
        2.  Check if string != RankSpec.ROOK.designation. If not, return failure.
        3.  If all checks pass the id in a success validation result.
    ...
    def verify_team_quota(
        ...
        # ACTION:
        1.  Verify candidate is a safe number using number_validator. If so convert to number. Else return failure.
        2.  Check if ransom != RankSpec.ROOK.team_quota. If not, return failure.
        3.  If all checks pass the id in a success validation result.
