...
class DiagonalSpan:
    ...
    def compute(cls, origin: Coord) -> ComputationResult[List[Coord]]:
        span: List[Coord] = []
        # return ComputationResult(span)
        
        compute_result = DiagonalRay.compute(
            start_x=0,
            end_x=origin.column,
            x_step=1,
            end_y=origin.row,
        ...
        if compute_result.is_failure:
            return ComputeResult.failure(
                DiagonalSpanComputationFailedException(
                    f"{method}: {DiagonalSpanComputationFailedException.DEFAULT_MESSAGE}"
                )
            )
        compute_result = DiagonaRay.compute(
            start_x=0,
            end_x=origin.column,
            x_step=1,
            end_y=origin.row,
