...
class DomainVisitorSearch(Search[Domain, Piece]):
    ...
    def search(cls, data_owner: Domain, search_context: VisitorSearchContext) -> SearchResult[List[Piece]]:
        ...
        try:
            ...
            if search_context.id is not None:
                return cls._id_search(domain=data_owner, id=search_context.id)
            
            if search_context.name is not None:
                return cls._name_search(domain=data_owner, name=search_context.name)
            
            if search_context.coord is not None:
                return cls._coord_search(domain=data_owner, coord=search_context.coord)
            
            if search_context.rank_name is not None:
                return cls._rank_name_search(domain=data_owner, coord=search_context.coord)
            
            if search_context.ransom is not None:
                return cls._ransom_search(domain=data_owner, coord=search_context.coord)
            
            if search_context.team_id is not None:
                return cls._team_id_search(domain=data_owner, coord=search_context.coord)
            
            if search_context.team_name is not None:
                return cls._team_name_search(domain=data_owner, coord=search_context.coord)
    ...
    def _team_id_search(cls, domain: Domain, id: int) -> SearchResult[List[Piece]]:
        ...
        try:
            matches = [visitor for visitor in domain.visitor if visitor.team.id == id]
            if len(matches) == 0:
                return SearchResult.empty()
    ...
    def _team_name_search(cls, domain: Domain, name: str) -> SearchResult[List[Piece]]:
        ...
        try:
            matches = [visitor for visitor in domain.visitor if visitor.team.name.uppper() == name.upper()]
            if len(matches) == 0:
                return SearchResult.empty()
