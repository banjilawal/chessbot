...
from chess.operation import CaptureContext
from chess.piece import  PieceValidator, PieceValidationException, CombatantPiece

from chess.operation.exception import (
    ...
    def validate(t: CaptureContext) -> Result[CaptureContext]:
        ...
        try:
            ...
            if not piece_validation.is_success():
                raise PieceValidationException(f"{method}: the piece is invalid")

            enemy_validation = PieceValidator.validate(context.enemy)
            if not enemy_validation.is_success():
                raise PieceValidationException(f"{method}: the enemy is not a valid piece")
