...
from chess.formation import BattleOrderContext, BattleOrderContextBuilder, BattleOrderContextValidator
...
class BattleOrderService(EntityService[BattleOrderContext]):
    """
    ...
    _order_validator: BattleOrderContextService
    ...
    def __init__(
            ...
            builder: BattleOrderContextBuilder = BattleOrderContextBuilder(),
            validator: BattleOrderContextValidator = BattleOrderContextValidator(),
    ):
    ...
    @property
    def context_builder(self) -> BattleOrderContextBuilder:
        return cast(BattleOrderContextBuilder, self.context_builder)
    
    @property
    def context_validator(self) -> BattleOrderContextValidator:
        return cast(BattleOrderContextValidator, self.entity_validator)
    ...
    def _find_by_name(
        ...
        method = "BattleOrderService._find_by_name"
        try:
    ...
    def _find_by_square(
        ...
        method = "BattleOrderService._find_by_square"
        try:
            validation = identity_service.validate_name(candidate=name)
            if validation.is_failure:
                return SearchResult.failure(validation.exception)
