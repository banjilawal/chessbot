...
class TeamContextValidationRouteException(ResultException, UnhandledRouteException):
    """
    # ROLE: Fallback Result, Debugging

    # RESPONSIBILITIES:
    1.  Indicate that TeamContextValidator did not handle one of the paths necessary to assure a candidate is a
        safe to use TeamContext. There are different configurations of TeamContext that are correct. Each
        configuration must have a testing route for a thorough verification process. If a TeamContext configuration
    ...
    DEFAULT_MESSAGE = (
        "The TeamContextValidator did not handle one of the paths necessary to assure a candidate is a safe to use"
        "TeamContext. Ensure all possible verification branches are covered to ensure the execution flow does not "
