...
class ChessPieceBuilder:
    ...
    def create(chess_piece_id: int, team: Team, category: ChessFigureCategory):
        name = team.color + "_" + category
        figure_rank = None
        number_of_rank_members = 0

        if category == ChessFigureCategory.KING:
            figure_rank = KingRank(KingMovement());
            number_of_rank_members = 1

        if category == ChessFigureCategory.QUEEN:
            figure_rank= QueenRank(QueenMovement())
            number_of_rank_members = 1

        if category == ChessFigureCategory.CASTLE:
            figure_rank = CastleRank(CastleMovement())
            number_of_rank_members = 2

        if category == ChessFigureCategory.BISHOP:
            figure_rank = BishopRank(BishopMovement())
            number_of_rank_members = 2

        if category == ChessFigureCategory.KNIGHT:
            figure_rank = KnightRank(KnightMovement())
            number_of_rank_members = 2

        if category == ChessFigureCategory.PAWN:
    ...
    def add_team_members(self, chess_figure_category: ChessFigureCategory, chess_rank: ChessRank,
