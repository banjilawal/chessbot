...
Usage:
  ...
  build_result = TeamBuilder.builder(visitor_team_id=1, player_agent=black_commander, team_schema=TeamProfile.BLACK)
...
Args:
  ...
  `player_agent`(`PlayerAgent`): The human or cybernetic moving pieces in `Team.roster`. The player_agent must pass
...
Example:
  ...
  notification = TeamBuilder.builder(visitor_team_id=1, player_agent=black-player_agent, team_schema=black_team_profile)
  if notification.is_success():
    team_name = cast(Team, notification.payload) # Guaranteed valid Team

  # Null player_agent will fail gracefully
  notification = TeamBuilder.builder(visitor_team_id=1, player_agent=None, team_schema=black_team_profile)
...
"""
"""
# ROLE: EntityService, Coordination
...
PARAMETERS:
    * `player_agent` (`PlayerAgent`): owner of `Team` object.
