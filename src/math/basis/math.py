...
class BasisTargetVectorSpanner(Function):
    """
    Role:
        -   Computation Worker
        -   Integrity Assurance
    ...
    def execute(self) -> ComputationResult[TargetVectorSet]:
        ...
        if validation.is_failure:
            ...
            return ComputationResult.failure(
                BasisTargetVectorSpannerException(
                    cls_mthd=method,
                    cls_name=self.__class__.__name__,
                    msg=BasisTargetVectorSpannerException.MSG,
                    err_code=BasisTargetVectorSpannerException.ERR_CODE,
                    mthd_rslt_type=MethodResultType.COMPUTATION_RESULT,
                    ex=validation.exception,
                ),
            )
        # --- Cast the validation product for additional processing ---#
        basis = cast(BasisSpace, validation.payload)
        ...
        origin = basis.origin
        for movement_vector in basis.maneuver_vectors.iterator:
