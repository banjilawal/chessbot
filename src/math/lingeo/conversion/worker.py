...
class LinGeoConverter(Worker):
    ...
    def work(
        ...
        if context.vector is not None:
            conversion_result = cls._convert_vector_to_coord(
                operand=context.vector,
                coord_service=tool_set.coord_service,
                vector_service=tool_set.vector_service,
                number_validator=tool_set.number_validator,
            )
        if context.coord is not None:
            conversion_result = cls._convert_coord_to_vector(
                operand=context.vector,
                coord_service=tool_set.coord_service,
                vector_service=tool_set.vector_service,
                number_validator=tool_set.number_validator,
