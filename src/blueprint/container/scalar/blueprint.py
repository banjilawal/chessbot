# src/blueprint/container/scalar/blueprint.py

"""
Module: blueprint.container.scalar.blueprint
Author: Banji Lawal
Created: 2026-04-03
version: 1.0.1
"""

from __future__ import annotations
from dataclasses import dataclass
from typing import Type

from err import ScalarNullException
from container import Blueprint, Scalar

@dataclass
class ScalarBlueprint(ContainerBlueprint[Scalar]):
    """
    Role:
        -   Container

    Responsibilities:
        1.  Provides magnitude value for instantiating a Scalar object.

    Attributes:
        magnitude: int
        container_type: Scalar
        null_exception: ScalarNullException
    Provides:

     Super Class:
        ContainerBlueprint
     """
    magnitude: int
    null_exception: ScalarNullException = ScalarNullException()
    owner: Scalar = Type[Scalar]
    owner_name: str = type(owner).__name__
