...
class GameDisplay:
    ...
    def draw_entity(self, entity: 'GridEntity'):
        ...
        if entity is None:
            print("[Warning] Entity cannot be None. Cannot draw a null piece to the screen.")
            return
    ...
    def get_entity_at_mouse_position(self, mouse_position: tuple) -> Optional['GridEntity']:
        if mouse_position is None:
            print("[Warning] Mouse position cannot be None. Cannot get an piece at a null position.")
            return None
        coordinate = self.grid_coordinate_at_mouse_position(mouse_position)
        if coordinate is None:
            print("Mouse is outside the game board. Cannot get an piece at a position outside the board.")
            return None
    ...
    def start_drag(self, mover: Mover, mouse_position: tuple[int, int]) -> None:
        ...
        print("piece", mover.mover_id, "dragging started at", self.active_drags[mover.mover_id].original_coordinate)
    ...
    #
    #     # Enforce HorizontalMover constraint
    #     if isinstance(drag_state.piece, HorizontalMover):
    #         new_row = drag_state.original_coordinate.row
    #
    #     # Prevent going beyond right/bottom edges
    #     new_col = min(new_col, self.board.dimension.length - drag_state.piece.dimension.length)
    #     new_row = min(new_row, self.board.dimension.height - drag_state.piece.dimension.height)
    #
    #     try:
    ...
    #           f"Valid: {self.board.can_entity_move_to_cells(drag_state.piece, new_coord)}")
    #
    #     # Only update if position changed and is valid
    #     if (new_coord != drag_state.current_coordinate and
    #             self.board.can_entity_move_to_cells(drag_state.piece, new_coord)):
    ...
    #
    #     if isinstance(self.active_drags[mover_id_counter].piece, HorizontalMover):
    ...
    #         return
    #
    #     if self.board.can_entity_move_to_cells(drag_state.piece, new_coordinate):
    #         self.active_drags[mover_id_counter] = self.active_drags[mover_id_counter].with_updated_position(new_coordinate)
    #         print("piece", mover_id_counter, "dragging updated to", self.active_drags[mover_id_counter].current_coordinate)
    #     else:
    ...
    def move_handler(self, entity: GridEntity, destination_coordinate: GridCoordinate) -> bool:
        if entity is None:
            print("[Warning] Entity cannot be None. Cannot move null piece.")
            return False
        if destination_coordinate is None:
            print("[Warning] Destination top_left_coordinate cannot be None. Cannot move piece without a destination top_left_coordinate.")

        if self.board is None:
            print("[Warning] PodBoard cannot be None. Cannot move on a nonexistent board.")
            return False
        if entity.top_left_coordinate is None:
            print("[Warning] Entity has no top_left_coordinate. Cannot move an piece without a top_left_coordinate.")
            return False
        if entity.top_left_coordinate is None:
            print("[Warning] Entity has no top_left_coordinate. Cannot move an piece without a top_left_coordinate.")
            return False
        if not isinstance(entity, HorizontalMover):
            print(f"[Warning] Entity {entity.id} is not a horizontal piece. Cannot move.")
            return False

        print(f"Debug: Moving piece {entity.id}")
