# src/operand/register/entity/node/blueprint/operand.py

"""
Module: operand.register.entity.node.blueprint.operand
Author: Banji Lawal
Created: 2026-04-03
version: 1.0.1
"""

from __future__ import annotations

from typing import Type, cast

from blueprint import NodeBlueprint
from err import NodeBlueprintNullException
from operand import EntityRegister


class NodeBlueprintEntityRegister(EntityRegister[NodeBlueprint]):
    """
    Role:
        -   Operand
        -   Data Holder

    Responsibilities:
        1.  Contains the operand and null_exception for an entity.

    Attributes:
        operand: NodeBlueprint
        null_exception: NodeBlueprintNullException
            
    Provides:

    Super Class:
    """
    
    def __init__(
            self,
            operand: NodeBlueprint = Type[NodeBlueprint],
            null_exception: NodeBlueprintNullException = NodeBlueprintNullException(),
    ):
        """
        Args:
            operand: Operand
            null_exception: NullException
        """
        super().__init__(operand=operand, null_exception=null_exception)
        
    @property
    def operand(self) -> NodeBlueprint:
        return cast(NodeBlueprint, self.operand)
    
    @property
    def null_exception(self) -> NodeBlueprintNullException:
        return cast(NodeBlueprintNullException, self.null_exception)
    
    @property
    def is_node_blueprint_entity_register(self) -> bool:
        return isinstance(self, NodeBlueprintEntityRegister)
    
    def __eq__(self, other):
        if other is self: return True
        if other is None: return False
        if isinstance(other, NodeBlueprintEntityRegister):
            return (
                    self.operand == other.operand and
                    self.null_exception == other.null_exception
            )
    
