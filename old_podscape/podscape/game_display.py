...
class GameDisplay:
    ...
    def draw_entity(self, entity: 'GridEntity'):
        ...
        if entity.top_left_coordinate is None:
            print("[Warning] Entity has no top_left_coordinate. Cannot draw an chess_piece without a top_left_coordinate to the screen.")
            return

        horizontal_mover_color = PodscapeColor.OLIVE.value
        vertical_mover_color = PodscapeColor.DEEP_ORANGE.value
        # print(f"Drawing chess_piece {chess_piece.mover_id_counter} at top_left_coordinate {chess_piece.top_left_coordinate}")
        ...
        # Draw the chess_piece (fixed the width parameter)
    ...
    def get_entity_at_mouse_position(self, mouse_position: tuple) -> Optional['GridEntity']:
        if mouse_position is None:
            print("[Warning] Mouse position cannot be None. Cannot get an chess_piece at a null position.")
            return None
    ...
    def start_drag(self, mover: Mover, mouse_position: tuple[int, int]) -> None:
        ...
        print("chess_piece", mover.mover_id, "dragging started at", self.active_drags[mover.mover_id].original_coordinate)
    ...
    #
    #     # Enforce HorizontalMover constraint
    #     if isinstance(drag_state.chess_piece, HorizontalMover):
    ...
    #
    #     if isinstance(self.active_drags[mover_id_counter].chess_piece, HorizontalMover):
    ...
    def move_handler(self, entity: GridEntity, destination_coordinate: GridCoordinate) -> bool:
        if entity is None:
            print("[Warning] Entity cannot be None. Cannot move null chess_piece.")
            return False
        ...
        if not isinstance(entity, HorizontalMover):
            print(f"[Warning] Entity {entity.id} is not a horizontal chess_piece. Cannot move.")
            return False

        print(f"Debug: Moving chess_piece {entity.id}")
