...
class CartesianDistanceTest(unittest.TestCase):
    ...
    def test_null_p_coord_raises_exception(self):
        with pytest.raises(NullCoordinateException):
           CartesianDistance(p=None, q=Coordinate(row=0, column=0))


    def test_null_p_row_raises_exception(self):
        with pytest.raises(NullRowException):
           CartesianDistance(p=Coordinate(row=None, column=0), q=Coordinate(row=0, column=0))


    def  test_p_row_below_lower_bounds_raises_exception(self):
        with self.assertRaises(RowOutOfBoundsException):
            CartesianDistance(p=Coordinate(row=-1, column=0), q=Coordinate(row=0, column=0))


    def  test_p_row_above_upper_bounds_raises_exception(self):
        with self.assertRaises(RowOutOfBoundsException):
            CartesianDistance(p=Coordinate(row=ROW_SIZE, column=0), q=Coordinate(row=0, column=0))
    ...
    def test_null_p_column_raises_exception(self):
        with pytest.raises(NullColumnException):
           CartesianDistance(p=Coordinate(row=0, column=None), q=Coordinate(row=0, column=0))


    def test_p_column_below_lower_bounds_raises_exception(self):
        with self.assertRaises(ColumnOutOfBoundsException):
            CartesianDistance(p=Coordinate(row=0, column=-1), q=Coordinate(row=0, column=0))

    def test_p_column_above_upper_bounds_raises_exception(self):
        with self.assertRaises(ColumnOutOfBoundsException):
            CartesianDistance(p=Coordinate(row=0, column=COLUMN_SIZE), q=Coordinate(row=0, column=0))
    ...
    def test_null_q_coord_raises_exception(self):
        with pytest.raises(NullCoordinateException):
            CartesianDistance(p=Coordinate(row=0, column=0), q=None)


    def test_null_q_row_raises_exception(self):
        with pytest.raises(NullRowException):
            CartesianDistance(p=Coordinate(row=0, column=0), q=Coordinate(row=None, column=0))


    def test_q_row_below_lower_bounds_raises_exception(self):
        with self.assertRaises(RowOutOfBoundsException):
            CartesianDistance(p=Coordinate(row=0, column=0), q=Coordinate(row=-1, column=0))


    def test_q_row_above_upper_bounds_raises_exception(self):
        with self.assertRaises(RowOutOfBoundsException):
            CartesianDistance(p=Coordinate(row=0, column=0), q=Coordinate(row=ROW_SIZE, column=0))
    ...
    def test_null_q_column_raises_exception(self):
        with pytest.raises(NullColumnException):
            CartesianDistance(p=Coordinate(row=0, column=0), q=Coordinate(row=0, column=None))


    def test_q_column_below_lower_bounds_raises_exception(self):
        with self.assertRaises(ColumnOutOfBoundsException):
            CartesianDistance(p=Coordinate(row=0, column=0), q=Coordinate(row=0, column=-1))

    def test_q_column_above_upper_bounds_raises_exception(self):
        with self.assertRaises(ColumnOutOfBoundsException):
            CartesianDistance(p=Coordinate(row=0, column=0), q=Coordinate(row=0, column=COLUMN_SIZE))
