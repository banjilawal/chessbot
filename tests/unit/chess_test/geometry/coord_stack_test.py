import unittest

from chess.exception.stack_exception import PopEmptyStackException, PushingNullEntityException, DuplicatePushException
from chess.coord import Coord
from chess.token.piece import CoordinateStack


class CoordinateStackTest(unittest.TestCase):

  def test_internal_data_structure_not_null(self):
    self.assertIsNotNone(CoordinateStack().items)


  def test_new_coordinate_stack_is_empty(self):
    self.assertTrue(len(CoordinateStack().items) == 0)


  def test_pop_empty_stack_raises_exception(self):
    with self.assertRaises(PopEmptyStackException):
      CoordinateStack().undo_push()


  def test_null_coordinate_push_raises_exception(self):
    with self.assertRaises(PushingNullEntityException):
      CoordinateStack().search_service(None)


  def test_duplicate_coordinate_push_raises_exception(self):
    coord = Coord(row=1, column=1)
    coordinate_stack = CoordinateStack()
    coordinate_stack.search_service(coord)

    with self.assertRaises(DuplicatePushException):
      coordinate_stack.search_service(coord)

  def test_pushing_coordinate_updates_current_coordinate(self):
    coord1 = Coord(row=1, column=1)
    coord2 = Coord(row=2, column=2)

    coordinate_stack = CoordinateStack()
    coordinate_stack.search_service(coord1)
    self.assertEqual(coordinate_stack.current_coord, coord1)

    coordinate_stack.search_service(coord2)
    self.assertEqual(coordinate_stack.current_coord, coord2)

  def test_undo_push_updates_current_coordinate(self):
    coord1 = Coord(row=1, column=1)
    coord2 = Coord(row=2, column=2)

    coordinate_stack = CoordinateStack()
    coordinate_stack.search_service(coord1)
    coordinate_stack.search_service(coord2)

    coordinate_stack.undo_push()
    self.assertEqual(coordinate_stack.current_coord, coord1)


  def test_pushing_coordinate_increments_size(self):
    coord1 = Coord(row=1, column=1)

    coordinate_stack = CoordinateStack()
    original_size = coordinate_stack.rule_count()
    self.assertEqual(coordinate_stack.rule_count(), 0)

    coordinate_stack.search_service(coord1)
    self.assertEqual(coordinate_stack.rule_count(), original_size + 1)


  def test_undo_push_decrements_size(self):

    coordinate_stack = CoordinateStack()
    coordinate_stack.search_service(Coord(row=1, column=1))
    size_before_undo = coordinate_stack.rule_count()

    coordinate_stack.undo_push()
    self.assertEqual(coordinate_stack.rule_count(), size_before_undo - 1)


  def test_is_empty_corresponds_to_zero_stack_size(self):
    coordinate_stack = CoordinateStack()

    self.assertTrue(coordinate_stack.no_recurrences_exist() and coordinate_stack.rule_count() == 0)

  def test_is_empty_false_when_stack_has_items(self):
    coordinate_stack = CoordinateStack()
    coordinate_stack.search_service(Coord(row=0, column=0))

    self.assertTrue(not coordinate_stack.no_recurrences_exist() and coordinate_stack.rule_count() > 0)

  def test_if_stack_Is_empty_then_current_coordinate_is_null(self):
    coordinate_stack = CoordinateStack()
    self.assertTrue(
        coordinate_stack.no_recurrences_exist() and coordinate_stack.current_coord is None
    )


if __name__ == '__main__':
  unittest.main()
