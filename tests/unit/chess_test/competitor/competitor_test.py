...
class CompetitorTest(unittest.TestCase):
  ...
  @staticmethod
  def valid_mock_competitor(competitor_id=1, name="owner"):
    ...
    competitor.visitor_name = "Valid Player"
    competitor.teams = sides_played

    return competitor


  @patch('assurance.notification.visitor_name.NameValidationProcess.validate')
  ...
  def test_competitor_failed_id_validation_raises_error(
    ...
    with self.assertRaises(IdValidationException):
      Commander(competitor_id=-1, name="owner")
